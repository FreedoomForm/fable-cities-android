...
// --- pedestrian sidewalk lanes (roads/RoadNetwork.js _buildLanes pedestrian block) ---
const PED_OFFS = { avenue: [10.6], local: [5.0] };   // highway/path have none
const pedLanes = [];
for (const seg of SEGS) {
  (PED_OFFS[seg.type] || []).forEach((off, rank) => {
    for (const dir of [1, -1]) {
      const dx = seg.pts[seg.pts.length - 1][0] - seg.pts[0][0];
      const dz = seg.pts[seg.pts.length - 1][1] - seg.pts[0][1];
      const dl = Math.hypot(dx, dz) || 1;
      const rx = -dz / dl, rz = dx / dl;
      const lat = off * dir;
      pedLanes.push({
        id: `${seg.id}:p${dir > 0 ? 'f' : 'r'}${rank}`, segmentId: seg.id, dir, rank,
        points: seg.pts.map(([x, y]) => ({ x: x + rx * lat, y: y + 0.2, z: 0 })),
        speed: 5, width: 1.5, from: dir > 0 ? seg.a : seg.b, to: dir > 0 ? seg.b : seg.a, kind: 'pedestrian',
      });
    }
  });
}
const pedConnections = new Map(pedLanes.map((l) => [l.id, []]));
for (const nodeId of Object.keys(NODES)) {
  const pIn = pedLanes.filter((l) => l.to === nodeId);
  const pOut = pedLanes.filter((l) => l.from === nodeId);
  for (const l of pIn) pedConnections.set(l.id, pOut.map((o) => o.id));
}
console.log('ped lanes', pedLanes.length, 'ped connections', [...pedConnections.values()].reduce((s, a) => s + a.length, 0));

const graph = {
  lanes: LANES,
  connections,
  pedestrian: { lanes: pedLanes, connections: pedConnections },
...
const pedLaneElems = net.pedLaneElems.join(',');
const pedOut = net.pedElements.map((el) => ({
  kind: el.kind, id: el.id, crossing: el.crossing ? 1 : 0, len: f9(el.poly.len),
  node: el.node ?? '', outs: el.outs.join(','),
}));
...
const randomPedLaneProbes = [0, 0.17, 0.34, 0.51, 0.68, 0.85, 0.99].map((r) => net.randomPedLane(r));
...
const pedsMade = sim.spawnPeds(10);
console.log('spawned', made, 'fleet', sim.vehicles.length, 'peds', pedsMade);
...
const pedSnap = () => sim.peds.map((p) => ({
  id: p.id, e: p.elem, s: f9(p.s), v: f9(p.v), x: f9(p.x), y: f9(p.y), z: f9(p.z),
  yaw: f9(p.yaw), ph: f9(p.phase), d: f9(p.dist),
}));
const checkpoints = [];
for (let f = 1; f <= 900; f++) {
  ...
  if (f === 150 || f === 300 || f === 480 || f === 620 || f === 900) {
    checkpoints.push({ f, despawned: f === 480 ? despawned : undefined, fleet: sim.vehicles.length, veh: vehSnap(), claims: claimSnap(), peds: pedSnap() });
  }
}
...
const pedLanesK = pedLanes.map((l) =>
  `        Traffic.GraphLane("${l.id}", ${ptsK(l.points)}, ${num(l.speed)}, "${l.segmentId}", ${l.dir}, "${l.from}", "${l.to}", ${num(l.width)})`).join(',\n');
const pedConnsK = pedLanes.map((l) => {
  const outs = pedConnections.get(l.id) || [];
  return `        "${l.id}" to listOf(${outs.map((o) => `"${o}"`).join(', ')})`;
}).join(',\n');
...
${elements.map((e) => `        ElG(${e.kind}, "${e.id}", ${e.turn}, ${e.rank}, ${e.fromLane}, ${e.toLane}, ${e.localIdx}, ${e.yieldDelay.toFixed(9)}, ${e.speed.toFixed(9)}, ${e.len.toFixed(9)}, ${e.vmax0.toFixed(9)}, ${e.vmaxM.toFixed(9)}, ${e.vmaxL.toFixed(9)}, ${e.sx.toFixed(9)}, ${e.sz.toFixed(9)}, ${e.ex.toFixed(9)}, ${e.ez.toFixed(9)}, listOf(${e.outs || ''}))`).join(',\n')},
    ...
    val pedElementCount = ${net.pedElements.length}
    val pedLaneElemCount = ${net.pedLaneElems.length}
    val pedLaneElemList = listOf(${pedLaneElems || ''})
    val pedG = listOf(
${pedOut.map((e) => `        PedElG(${e.kind}, "${e.id}", ${e.crossing}, ${e.len.toFixed(9)}, "${e.node}", listOf(${e.outs || ''}))`).join(',\n')},
    )
    val randomPedLaneProbes = listOf(${randomPedLaneProbes.join(', ')})
    val pedCum = floatArrayOf(
${'        ' + [...net.pedCum].map((v) => `${f9(v)}f`).join(', ')},
    )
    const val pedTotal = ${net.pedTotal.toFixed(9)}
    const val pedsSpawned = ${pedsMade}
    val nodeG = listOf(
...
const graphJson = { lanes: graph.lanes, connections: Object.fromEntries(graph.connections), pedestrian: { lanes: graph.pedestrian.lanes, connections: Object.fromEntries(graph.pedestrian.connections) }, nodePos: graph.nodePos, segType: graph.segType };
